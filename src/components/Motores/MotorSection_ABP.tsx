...
export default function MotorSection_ABP({ result }: Props) {
 ...
 🚀 Proyecto ABP
      ...
      {(p.fases || []).map((f, idx) => (
        <div key={f.nombre ?? `fase-${idx}`} style={{ marginBottom: '0.5rem', paddingLeft: '0.5rem', borderLeft: '2px solid #2563EB', fontSize: '0.75rem' }}>
          <div style={{ fontWeight: 600 }}>{f.nombre} — {f.duracion}</div>
          {(f.actividades || []).map((a) => (
            <div key={a} style={{ color: '#4a4a5a' }}>• {a}</div>
          ))}
        </div>
      ))}
      <div style={{ fontSize: '0.75rem', fontWeight: 600, marginTop: '0.5rem' }}>Productos:</div>
      {(p.productos || []).map((pr, idx) => (
        <div key={pr ?? `prod-${idx}`} style={{ fontSize: '0.75rem', color: '#4a4a5a' }}>📦 {pr}</div>
      ))}
      {p.evaluacion && (
