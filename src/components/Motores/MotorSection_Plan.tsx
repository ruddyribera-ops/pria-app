...
export default function MotorSection_Plan({ result }: Props) {
  ...
  try {
    const sd = (result as any)?.secuencia_didactica;
    if (!sd) return null;
    return (
      <div style={{ borderTop: '1px solid #e6e6eb', padding: '1rem' }}>
        ...
        <div style={{ fontSize: '0.6875rem', color: '#6b6b80' }}>
          Inteligencias: {(result as any)?.inteligencias_multiples?.map((i: any) => i.inteligencia).join(', ')}
        </div>
      </div>
    );
  } catch { return null; }
