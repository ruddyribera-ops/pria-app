    ...
    <div style={{ borderTop: '1px solid #e6e6eb', padding: '1rem' }}>
      ...
      <div style={{ fontSize: '0.75rem', color: '#4a4a5a' }}>
        ...
        <div style={{ marginBottom: '0.5rem' }}>
          <strong>Puntos clave:</strong> {(pt.puntos_clave || []).map((p) => (
            <span key={p} style={{ display: 'inline-block', background: '#e0f2fe', borderRadius: '4px', padding: '0.125rem 0.5rem', margin: '0.125rem' }}>{p}</span>
          ))}
        </div>
        {pt.momentos_criticos && pt.momentos_criticos.length > 0 && (
          <div style={{ marginBottom: '0.5rem' }}>
            <strong> Momentos críticos:</strong>
            {pt.momentos_criticos.map((m) => (
              <div key={m.momento} style={{ paddingLeft: '0.5rem', borderLeft: '2px solid #0891B2', marginTop: '0.25rem' }}>
        ...
        {pt.adaptaciones_rapidas && pt.adaptaciones_rapidas.length > 0 && (
          <div>
            ...
            {pt.adaptaciones_rapidas.map((a) => (
              <div key={a.diagnostico} style={{ color: '#9333EA' }}>→ {a.diagnostico}: {a.intervencion}</div>
            ))}
          </div>
        )}
      </div>
