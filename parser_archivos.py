...
def _classify_block(valor: str) -> tuple[str, str | None, str | None, str | None]:
    ...
    materia = materia_raw.strip().title() if materia_raw.strip() else valor.strip().title()
...
def parse_calendario(wb_bytes: bytes) -> list[dict]:
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            ...
            item      = str(row[fecha_col + 1]).strip() if fecha_col + 1 < len(row) and row[fecha_col + 1] else ''
            actividad = str(row[fecha_col + 2]).strip() if fecha_col + 2 < len(row) and row[fecha_col + 2] else ''
            obs       = str(row[fecha_col + 3]).strip() if fecha_col + 3 < len(row) and row[fecha_col + 3] else ''
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(values_only=True):
            ...
            if len(row) >= 4:
                ...
                actividad = str(actividad_candidate).strip() if actividad_candidate else None
