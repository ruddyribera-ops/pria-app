...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title=f"PRIA v{app_config.app_version} — Método Palma-Ribera",
    layout="wide",
    page_icon="🦉",
)
...
def analizar_pdf_ocr(pdf_bytes: bytes, filename: str) -> dict:
    ...
    # Use centralized prompt loading with usage tracking
    prompt_base = load_motor_prompt("Motor_Alpha-2")
    if not prompt_base:
        return {**VACIO, "unidad_real": "Error cargando Motor_Alpha-2"}
...
def generar_con_gemini(
    ...
    # Extract motor name from filename (e.g., "Motor_M0a.txt" -> "Motor_M0a")
...
with st.sidebar:
    ...
    if ss.diagnosticos_texto:
        ...
        if st.button(
            "🔄 Releer diagnósticos", use_container_width=True, key="btn_releer_diag"
        ):
                ...
                if f_vig and st.button(
                    ...
                    with st.spinner("Extrayendo Inteligencia..."):
                        try:
                            ...
                            _prompt = """Analiza el documento de vigilancias de recreo y extrae EXACTAMENTE esta estructura para CADA docente:

{"nombre_hoja": "NOMBRE_EXACTO_EN_MAYUSCULA", "ubicacion": "DETALLE_COMPLETO"}

Ejemplo de formato de ubicacion CORRECTO:
- "PRIMARIA RECREO 1 LUNES PATIO CENTRAL"
- "SECUNDARIA RECREO 2 MARTES ESCALERAS B"
- "PRIMARIA RECREO 1 JUEVES KIOSCO"

IMPORTANTE:
1. Usa nombres EXACTOS de la hoja de cálculo (VANESA, RUDDY, SUSI, etc.)
2. Incluye el número de recreo (1 o 2), el día (LUNES/MARTES/MIERCOLES/JUEVES/VIERNES), y la ubicación específica
3. Si un docente tiene múltiples vigilancias, sepáralas con coma: "PRIMARIA RECREO 1 LUNES PATIO, PRIMARIA RECREO 2 MIÉRCOLES KIOSCO"
4. Devuelve SOLO un array JSON, sin texto adicional"""
                        ...
                        if not _plan_txt.strip():
                            ...
                            if isinstance(_plan_data, dict):
                                _plan_data = _plan_data.get(
                                    "bloques", _plan_data.get("clases", [_plan_data])
                                )
                            ...
                            for _blk in _plan_data:
                                ...
                                if isinstance(_tasks_raw, list):
                                    for t in _tasks_raw:
                                        if isinstance(t, str):
                                            _tasks.append(t.strip())
                                        elif isinstance(t, dict):
                                            _tasks.append(
                                                str(list(t.values())[0]).strip()
                                            )
...
if zona == "🌅  Diario":
    ...
    render_daily_view(
        ss=ss,
        _nombre_hoja=_nombre_hoja,
        _fecha_iso=_fecha_iso,
        _dia_es=_dia_es,
        _dow=_dow,
        _hoy=_hoy,
