...
def render_sidebar():
    ...
    grado_options = [
        f"{g}{sufijos.get(g, 'mo')} {nivel.lower()}" for g in range(1, max_grado + 1)
    ]
    sel_idx = next(
    ...
    if os.path.exists(CACHE_DIR):
        all_cache = [f for f in os.listdir(CACHE_DIR) if f.endswith(".json")]
        motor_cache = [f for f in all_cache if f.startswith("motor_")]
        pdf_cache = [f for f in all_cache if not f.startswith("motor_")]
    else:
        all_cache = motor_cache = pdf_cache = []

    if all_cache:
        st.divider()
        parts = []
        if motor_cache:
            parts.append(f"{len(motor_cache)} motores")
        if pdf_cache:
            parts.append(f"{len(pdf_cache)} PDFs")
        st.markdown(f"**📦 Caché:** {', '.join(parts)}")
        col_m, col_p = st.columns(2)
        with col_m:
            if motor_cache and st.button("🗑️ Motores", use_container_width=True, help="Borrar respuestas de motores IA en caché"):
                limpiar_motor_cache()
                st.rerun()
        with col_p:
            if pdf_cache and st.button("🗑️ PDFs", use_container_width=True, help="Borrar PDFs y documentos en caché"):
                for cf in pdf_cache:
                    try:
