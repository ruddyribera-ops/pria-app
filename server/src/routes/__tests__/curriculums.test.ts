...
describe('Curriculums routes', () => {
  ...
  test('GET /curriculums returns list of curriculums', async () => {
    const res = await fetch(`http://127.0.0.1:${port}/api/curriculums`, {
  ...
  test('POST /curriculums creates a curriculum record', async () => {
    const res = await fetch(`http://127.0.0.1:${port}/api/curriculums`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        unidad_real: 'Unidad 1 — Matematicas',
        temas: ['Numeros', 'Operaciones'],
        contenido_temas: { 'Numeros': 'Contenido sobre numeros', 'Operaciones': 'Contenido sobre operaciones' },
        paginas_temas: { 'Numeros': 'pp. 1-5', 'Operaciones': 'pp. 6-10' },
      }),
    });
    expect(res.status).toBe(200);
    const data = await res.json() as any;
